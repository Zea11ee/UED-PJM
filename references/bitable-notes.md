...
## 任务查询准则（WBS）
- 人员“目前/当前/手上/在做/待处理”任务：负责人包含目标人员，状态 != 已完成，状态 != 废弃；不要把“手上”解释为仅“未启动”。
- 人员任务查询必须全量分页；`record-list` 分页结果不代表全表，不能只取第一页或首批匹配。
- 若未关闭结果中有历史遗留，优先呈现当前/近期任务，并把明显过期或日期为空的记录单独标为“历史未清理”。
- 任务源只作为父任务上下文补充；WBS 为空时才考虑用任务源/飞书任务作为主列表。
