...
## 11) 近期补充经验（2026-03-03）
- 先定任务类型，再拆 WBS：轻量配置/联调（2-4条）、活动双阶段（Teasing/Launch）、问题修复（修复+验收起步）。
- 节点优先：先锁上线日期，再倒排前置任务，优先保证 `PRD -> 设计/开发 -> 测试/验收 -> 上线走查` 的闭环。
- 依赖强约束：PRD 必须先于开发；设计联调与开发可并行；上线走查必须安排在上线当天。
- 时间表达统一：所有“今天/明天”等相对日期在落库前转换为绝对日期（Asia/Shanghai）。
- 工时策略：复用型页面优先按“复用量 + 新增模块 + 验收返修成本”估算；测试反馈修改建议独立建条。
- 流程门槛：先出表格草案（父任务 + WBS）再写库，避免直接落库导致返工。
- 稳定性控制：按源编号去重；写入后按源编号和父子关联回查，必须返回 record_id。
- 附件处理：跨 Base 的 file_token 可能不可写；失败时在描述记录来源记录号/链接，不阻塞任务落库。

## 12) 知识库机制（新增）
- 新增“项目知识库 + 案例记忆库 + 人员档案库”三层结构，用于替代高频全量历史检索：
  - 项目知识库：`references/project-kb/*.md`（项目特有分工、模块变更沉淀）。
  - 案例记忆库：`references/case-kb/*.md`（按需求类型的可复用拆分模板）。
  - 人员档案库：`references/people-profiles.md`（name/open_id/user_id/email/role）。
- 执行顺序建议：`项目KB/案例KB -> 历史检索补充 -> 草案确认 -> 落库 -> 回查 -> 反写项目KB`。
- 落库后必须追加“变更事件”到项目知识库：记录日期、需求编号、模块、功能变化、状态、关联记录ID。
- 人员档案库仅用于提效，contacts MCP 仍是最终校验来源。
